...
Это «out-of-band» команды управления плагином и живой сессией из Telegram. Плагин перехватывает их *до* того, как они дойдут до Claude обычным промптом — поэтому `/status` не будит модель, а keystroke-команды (`/keys`, `/cc`, `/stop`, `/reset`, `/new`) напрямую управляют tmux-pane сессии агента. Регистрируются через `setMyCommands`, поэтому видны в меню «/» Telegram (описания локализованы на русский, PR #18).

Полный авторитетный список — в коде: `plugin/src/commands/oob.ts` (`helpText()` + `BOT_COMMANDS` + union `OobCommandName`) и `plugin/src/commands/keys.ts` (whitelist клавиш панели `/keys` и passthrough `/cc`).
...
| `/keys` | Открывает панель инлайн-кнопок «в одно касание» из **whitelisted** клавиш для tmux-pane агента — так вы **отвечаете на нативный диалог Claude Code** из Telegram. Один тап = одна клавиша в сессию; панель не «расходуется» (тапайте многократно в многошаговом диалоге). Есть `⌫ backspace` и `🧹 clear`. | `/keys`, затем тап по кнопке. Простой способ ответить на диалог. |
...
- **`/keys`** → тап по кнопке. Панель — одна клавиша на тап, раскладка:
  ...
  По этой же панели можно тапать многократно в многошаговом диалоге — она не «расходуется». Тап `1` выбирает пункт 1, `3` — «No», `⎋ esc` отменяет.

Каждый тап инжектит ровно одну клавишу из закрытого whitelist в pane — текст-команды для клавиш нет, поэтому даже pane, выпавший в голый shell, нельзя заставить что-либо выполнить.

**Клавиши панели `/keys` (полный whitelist):**
...
Всё вне этого набора отвергается. Поскольку набор — закрытый whitelist без свободного текста, даже pane, выпавший из Claude в голый shell, нельзя заставить выполнить команду.
...
- `/keys`, `/cc` и pane-ветки `/stop`/`/reset`/`/new` требуют резолвимого tmux-pane. Без него (нет tmux-конфига / нет `$TMUX`) плагин отвечает, что pane недоступен.
